...
import {
  Link as LinkIcon, ExternalLink, MoreVertical, Copy as CopyIcon,
  PauseCircle, PlayCircle, Trash2, Wallet as WalletIcon,
} from "lucide-react";
...
export default function PaymentLinkCard({
  ...
  const origin = typeof window !== "undefined" ? window.location.origin : "https://paylynk.xyz";
  const publicUrl = `${origin}/p/${link.slug}`;

  const createdAt = link.createdAt instanceof Date ? link.createdAt : new Date(link.createdAt);
  ...
  const amountLabel =
    link.amountUsd == null || link.amountUsd === "" ? "Any amount"
      ...
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            <Chip startContent={<LinkIcon className="size-4" />} color={statusColor as any} variant="flat" size="sm" radius="sm">
              {link.status === "ACTIVE" ? "Active" : "Inactive"}
            </Chip>
            <Chip variant="flat" color={assetColor as any} size="sm" radius="sm">{link.asset}</Chip>
            <Chip variant="flat" color="secondary" size="sm" radius="sm">{chainLabel}</Chip>
          </div>
          ...
          {link.memo && (
            <div className="text-xs text-default-600 line-clamp-2">{link.memo}</div>
          )}
        </div>

        <Dropdown placement="bottom-end">
          <DropdownTrigger>
            <Button isIconOnly variant="light" size="sm" radius="sm" aria-label="More actions">
          ...
          <DropdownMenu aria-label="Payment link actions">
            <DropdownItem
              key="toggle"
              startContent={link.status === "ACTIVE" ? <PauseCircle className="size-4" /> : <PlayCircle className="size-4" />}
      ...
      <CardBody className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          <Tooltip content={publicUrl} placement="top">
            <span className="truncate text-sm max-w-[220px] sm:max-w-[340px]">{publicUrl}</span>
          </Tooltip>

          <Button isIconOnly size="sm" variant="light" onPress={handleCopyUrl} aria-label="Copy URL">
            <CopyIcon className="size-4" />
          </Button>
          <Button
            isIconOnly size="sm" variant="light" aria-label="Open link"
